...
# (Отключена жесткая перезапись DNS, чтобы не ломать systemd-resolved)
...
# (rmem_default и wmem_default удалены для избежания утечки памяти и OOM-Killer)
...
# (Сброс PREROUTING удален, чтобы не сломать порты x-ui)
...
# Немедленно запускаем прокси обратно, чтобы вернуть x-ui-pro из дауна
